...
The decay mode \BdToJpsiKS is well known \addref{gold-plated channel} as the gold-plated channel to measure
\CP violation in \Bd decays. With negligible contributions from \DCS
modes [\addref{DCS mode}], a $\ovE{\bquark}\to\ovE{\cquark}\cquark\ovE{\squark}$ transition
dominated by a single weak phase [\addref{single weak phase}], and an easy to reconstruct final state the
...
\info{Introduce $A_{\DG}$?}

Consider the vanishing decay width difference of the two mass eigenstates
...
\end{equation*}
%
is \texthl{mostly} dominated by measurements by \Babar and \Belle in the
...
global fit to the \CKM parameter space. As this fit can be performed \replace{with and
without the respective parameters included}{something better}, a $\chisq$ pull can be computed for
