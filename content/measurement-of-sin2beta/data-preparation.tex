...
the \LZeroDiMuon trigger lines, \ie show high transverse momentum tracks in the
five muon stations. No further \LZero criteria are required.
...
\enquote{biased} line. The \HLTOne \DiMuonHighMass does not affect the decay
time distribution, hence is called an \enquote{unbiased} line. All trigger
...
where events had to fulfil the trigger requirements of the unbiased \HLTOne
di-muon line and the biased \HLTTwo di-muon line, a gain of
$\SI{19.7}{\percent}$ in signal efficiency is measured by additionally allowing
events to pass the requirements defined by the \HLTOne \TrackMuon line.
...
\begin{table}
  \centering
  \begin{threeparttable}
    \caption{Stripping cuts applied in the reconstruction and selection of the $\Jpsi$, the $\KS$, and the $\Bd$ candidates\tnote{\S}.\todo{Ref to stripping cuts}}
...
\Cref{fig:measurement_of_sin2beta:data_preparation:offline_selection:before}
shows the mass distribution of the 2011 and 2012 data set and the fit
...
To reduce proton-pion mis-ID from background $\LbToJpsiLambda$ decays all pion
...
set to \texttt{SSD\textunderscore{CP}} with parameters as summarised in
