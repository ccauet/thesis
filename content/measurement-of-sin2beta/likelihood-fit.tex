...
\PDF \cite{cbset}, marginalised over the a priori unknown per-event mass
resolution. The \PDF is used to describe the distribution of the reconstructed
$\B$ candidates mass $\obsMass$ and is parametrised as
%
...
is implemented using cubic splines\addref{Cubic splines} and the resolution model
...
\end{equation}
%
with\info{Check conditional vert in function arguments}\missing{Definition of omega and omegabar as function of eta}
%
\begin{equation}
  \begin{split}
...
Gaussian kernel estimations and splines\addref{Cubic splines}. The later one is used here. Splines are
...
\SSpi mistag estimates.\todo{Decimal separator alignment}}
...
estimates $\eta_\text{\acs{OS}/\acs{SSpi}}$ and the reconstructed \Bd decay time.\todo{Decimal separator alignment}}
