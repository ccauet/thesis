...
offline selection. As we do expect the tagged and untagged candidates to behave
equally concerning the studied effect we proceed using all candidates available.
...
The efficiencies are time-dependent and, since we are mainly interested in the
...
the acceptance at the decay time limits, we assume the efficiency to be flat
...
selection is applied and from the remaining multiple (\acs{PV},\Bz) candidate pairs,
...
statistical uncertainties and as we do not expect any deviations of the decay
time distributions we include all untagged events in this study. The number of
...
events to ToyMC events is calculated and shown in
