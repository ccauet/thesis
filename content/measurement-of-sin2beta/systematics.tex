...
\replace{effects}{something better} of the fit model and influences of the flavour tagging, the decay time
...
performed. The theoretical distribution given in
...
\begin{multline}
  \Asym{\CP}{\text{exp}} = \\ 
    \CPAsymmetry 
    \frac{\mistag^{\Bd} - \mistag^{\Bdbar} + A_{\text{P}}^{\catOO} \Bigl(1 - \mistag^{\Bd} - \mistag^{\Bdbar}\Bigr) + \Bigl(1 + \mistag^{\Bd} - \mistag^{\Bdbar} + A_{\text{P}}^{\catOO} \bigl(\mistag^{\Bd} - \mistag^{\Bdbar}\bigr) \Bigr)}
...
basis assuming true \Bd and \Bdbar mesons. Then the \sweighted averages of
$\mistag^{\Bd} = \num{0.3869}$ and $\mistag^{\Bdbar} = \num{0.3777}$ are
incorporated while the values for $A_{\text{P}}^{\catOO}$ and \DMd are taken as
found in the nominal fit. This results in central values fully compatible with
the ones extracted from the nominal fit
%
\begin{equation}
  \begin{split}
    \SJpsiKS &= \num{0.713 +- 0.053} \eqcm \\
...
\end{equation}
%
In a second approach the number of tagged \Bd and \Bdbar signal candidates is
counted on an \sweighted sample to compute the time-integrated \CP asymmetry
%
\begin{equation}
  \begin{split}
...
\begin{multline}\label{eq:measurement_of_sin2beta:systematics:cross_checks:time_integrated}
  \SJpsiKS^{\text{int}} = \\
  \frac{ \Asym{\CP}{\text{int}} - ( 1 - \mistag^{\Bd} - \mistag^{\Bdbar}) A_{\text{P}} \Asym{\CP}{\text{int}} ( \mistag^{\Bd} - \mistag^{\Bdbar} )}
    {1 - \mistag^{\Bd} - \mistag^{\Bdbar} - A_{\text{P}} \Asym{\CP}{\text{int}} - \Asym{\CP}{\text{int}} ( \mistag^{\Bd} - \mistag^{\Bdbar} ) }
  \frac{1 - (\DMd \tau)^2}{ \sin (\DMd t_{\text{min}}) + \DMd \tau \cos (\DMd t_{\text{min}})} \eqcm
\end{multline}
%
leads to 
%
